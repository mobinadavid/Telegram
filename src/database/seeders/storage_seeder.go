...
func StorageSeeder() {
	...
	for _, storage := range storages {
		_, err := db.Exec(context.Background(), query, storage.FileName, storage.FilePath, storage.FileSize, storage.FileType, storage.MimeType, storage.UploaderID, storage.MessageID, storage.MessageReplyID, storage.ChatID, time.Now(), time.Now())
		if err != nil {
